...
Deno.serve(async req=>{
  ...
  for(let i=0;i<unique.length;i+=20){
    ...
    for(const row of candidates.filter((x:any)=>ids.includes(String(x.ali_product_id)))){
      const detail=byId.get(String(row.ali_product_id));if(!detail)continue;
      const nextPayload={...(row.source_payload||{}),detail};
      await db(`sf_candidates?id=eq.${row.id}`,{method:"PATCH",body:JSON.stringify({stage:"enriched",source_payload:nextPayload,updated_at:new Date().toISOString()})});enriched++;
    }
  }
