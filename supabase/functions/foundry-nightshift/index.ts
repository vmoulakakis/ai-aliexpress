...
async function authorized(req:Request){
  const provided=req.headers.get("x-foundry-token")||"";
  if(!provided)return false;
  const r=await db("sf_internal_config?select=value&key=eq.job_token&limit=1");
  const rows=await r.json().catch(()=>[]);
  return Array.isArray(rows)&&typeof rows[0]?.value==="string"&&provided===rows[0].value;
}
async function aliSearch(query:string, pageSize=35) {
  ...
  const params:Record<string,string>={method:"aliexpress.affiliate.product.query",app_key:appKey,sign_method:"hmac",timestamp:topTimestamp(),format:"json",v:"2.0",keywords:query,target_currency:"EUR",target_language:"EN",tracking_id:tracking,ship_to_country:"GR",page_no:"1",page_size:String(pageSize),delivery_days:"15",sort:"LAST_VOLUME_DESC",fields:"product_id,product_title,product_main_image_url,target_sale_price,target_original_price,discount,commission_rate,lastest_volume,evaluate_rate,promotion_link,product_detail_url,shop_id,shop_url"};
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    let discovered=0, queries=0, errors=0; const painStats:Record<string,unknown>={};
    for(const pain of (Array.isArray(pains)?pains:[])){
      ...
      for(const query of queryList){
        try{
          const products=await aliSearch(String(query),35); queries++; const seen=new Set<string>();
          const rows=products.map((p:any)=>{const id=String(p?.product_id??"");if(!/^\d+$/.test(id)||seen.has(id)) return null;seen.add(id);return {run_id:run.id,ali_product_id:id,stage:"discovered",source_payload:{pain_id:pain.id,pain_slug:pain.slug,pain_title:pain.title_el,query:String(query),solution_label_el:pain?.sourcing_rules?.solution_label_el??null,product:p},scores:{volume:Number(p?.lastest_volume)||0,evaluate_rate:p?.evaluate_rate??null}};}).filter(Boolean);
