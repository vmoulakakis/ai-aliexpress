-- Compatibility layer for existing foundry-search Edge API.
-- Keeps the public RPC signature unchanged while ranking CORE before verified LAB.
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path=public AS $$
  ...
  SELECT solution_id,pain_title,solution_title,image_url,price_eur,old_price_eur,discount_pct,
         ...
         affiliate_url,express,quality_score,gap_type
  FROM ranked
...
GRANT EXECUTE ON FUNCTION public.sf_search_cards(text,int), public.sf_featured_cards(int) TO anon, authenticated;
